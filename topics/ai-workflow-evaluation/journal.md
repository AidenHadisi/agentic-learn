...
## Weak Spots
- **Teaching-style note (carried from `ds-math-stats`)**: Prefers concrete enumerated examples over algebraic derivation. Lead with worked numbers; keep derivations short. Batch several application questions into one form rather than asking one at a time — that format got full engagement in Section 1.
- None subject-specific yet. Section 1 produced no wrong answers, so nothing to carry.
